...
interface Props {
  ...
  // Detalhes das operações financeiras a confirmar
  operationDetails?: string;
}

export function ChatInteractiveInput({ args, onAnswer, answered, answeredValue, operationDetails }: Props) {
  const options = args.options ? args.options.split("|").map(o => o.trim()) : [];
  const [selected, setSelected] = useState<string[]>([]);
  const [sliderValue, setSliderValue] = useState<number>(args.sliderMin ?? 0);

  const handleSubmit = (answer: string) => onAnswer(answer);

  // ── Já respondido ─────────────────────────────────────────────
  if (answered) {
    const confirmed = answeredValue === "Sim";
    return (
      <div className={`flex items-center gap-2 text-[11px] rounded-lg px-3 py-2 border ${
        confirmed
          ? "text-emerald-400 bg-emerald-950/20 border-emerald-900/40"
          : "text-zinc-400 bg-zinc-900/50 border-zinc-800/50"
      }`}>
        <Check className="w-3.5 h-3.5 shrink-0" />
        <span>{confirmed ? "Confirmado e registado" : "Cancelado"}</span>
      </div>
    );
  }
  ...
  if (args.inputType === "confirm") {
    return (
      <div className="flex flex-col gap-2.5 bg-zinc-900 border border-amber-900/30 rounded-xl p-3.5">
        {/* Detalhes das operações — é o mais importante */}
        {operationDetails && (
          <div className="flex flex-col gap-1.5 bg-zinc-950/60 rounded-lg p-2.5 border border-zinc-800">
            <div className="flex items-center gap-1.5 text-[9px] font-bold text-amber-400 uppercase tracking-wider">
              <AlertCircle className="w-3 h-3" />
              Operação a registar
            </div>
            {operationDetails.split("\n").map((line, i) => (
              line.trim() ? (
                <p key={i} className="text-[11px] text-zinc-200 leading-relaxed"
        ...
        <p className="text-[11.5px] text-zinc-300 leading-relaxed">{args.question}</p>
        <div className="flex gap-2">
          <Button size="sm" onClick={() => handleSubmit("Sim")}
            className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-semibold rounded-lg h-8">
            ✓ Confirmar
          </Button>
          <Button size="sm" variant="outline" onClick={() => handleSubmit("Não")}
  ...
  if (args.inputType === "multiple") {
    ...
    return (
      <div className="flex flex-col gap-2.5 bg-zinc-900 border border-zinc-800 rounded-xl p-3.5">
            ...
            return (
              <button key={opt} onClick={() => toggle(opt)}
                className={`flex items-center gap-2.5 w-full px-3 py-2 rounded-lg border text-[11px] font-medium transition-all text-left ${
                  isSelected ? "bg-zinc-700 border-zinc-500 text-zinc-100" : "bg-zinc-800 border-zinc-700 text-zinc-300 hover:bg-zinc-750 hover:border-zinc-600"
                }`}>
        ...
        <Button size="sm" disabled={selected.length === 0} onClick={() => handleSubmit(selected.join(", "))}
      ...
      <div className="flex flex-col gap-3 bg-zinc-900 border border-zinc-800 rounded-xl p-3.5">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-bold text-zinc-100">{sliderValue.toLocaleString("pt-MZ")} {unit}</span>
            <span className="text-[10px] text-zinc-500">{max.toLocaleString("pt-MZ")} {unit}</span>
          </div>
          <Slider min={min} max={max} step={step} value={[sliderValue]}
            onValueChange={([v]) => setSliderValue(v)} className="w-full" />
        </div>
        <Button size="sm" onClick={() => handleSubmit(`${sliderValue.toLocaleString("pt-MZ")} ${unit}`.trim())}
